...
// serves a leaf signed by Let's Encrypt "YE2" but omits the YE2 intermediate from the
// handshake, so Node/undici fails with UNABLE_TO_VERIFY_LEAF_SIGNATURE (browsers recover
// via AIA fetching). Supply the missing YE2 intermediate (http://ye2.i.lencr.org/) as an
// extra trust anchor. Remove once FESCO fixes their server chain.
...
// YE2 is issued by ISRG "Root YE", which FESCO also omits. Root YE chains to the
// long-trusted ISRG Root X2 (in Node's default store), so two intermediates suffice.
