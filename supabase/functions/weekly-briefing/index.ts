// supabase/functions/weekly-briefing/index.ts
...
Deno.serve(async (req: Request) => {
  ...
  for (const user of users) {
    try {
      ...
      if (!aiRes.ok) {
        console.error(`[weekly-briefing] OpenAI error for ${user.id}:`, await aiRes.text())
      ...
      if (insertErr || !briefing) {
        console.error(`[weekly-briefing] insert error for ${user.id}:`, insertErr)
      ...
      console.error(`[weekly-briefing] error for ${user.id}:`, errMsg)
