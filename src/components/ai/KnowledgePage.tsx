...
export function KnowledgePage({ onBack }: Props) {
  ...
  const pageTabs: { key: PageTab; label: string }[] = [
    { key: 'list',   label: t('knowledgeTitle') },
        ...
        <div className="flex gap-1 flex-1">
          {pageTabs.map(tab => (
            <button
              ...
              style={{
                background:  pageTab === tab.key ? 'var(--brand)' : 'transparent',
                color:       pageTab === tab.key ? 'white'        : 'var(--ink-3)',
                borderColor: pageTab === tab.key ? 'var(--brand)' : 'transparent',
              }}
            >
              {tab.label}
            </button>
